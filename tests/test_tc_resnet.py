...
from src.models.tc_resnet import SpeechCommandTCResNet, TemporalResidualBlock, build_model


class TCResNetTests(unittest.TestCase):
    def test_output_shape(self) -> None:
        model = SpeechCommandTCResNet(num_classes=6, n_mels=64)
    ...
    def test_rejects_invalid_input_rank(self) -> None:
        model = SpeechCommandTCResNet(num_classes=6)
    ...
    def test_rejects_even_kernel_size(self) -> None:
        with self.assertRaises(ValueError):
            TemporalResidualBlock(8, 8, kernel_size=4)

    def test_backward_pass_produces_gradients(self) -> None:
        model = SpeechCommandTCResNet(
            num_classes=6,
            n_mels=64,
            channels=(8, 12),
